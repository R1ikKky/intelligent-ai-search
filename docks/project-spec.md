...
4. Объяснять, почему выдача изменилась.
...
2. Нужно хранить исходные варианты имён в `jsonb` для трассировки и объяснения качества данных.
...
flowchart LR
    ...
    subgraph SearchContour[Поисковый контур]
      ...
      S4 --> S5[Персонализация и объяснения]
...
sequenceDiagram
    ...
    API->>DB: Профиль пользователя + события
    API->>DB: Группировка и explanation factors
    API-->>FE: Группы результатов + explanations + recommendations
...
stateDiagram-v2
    ...
    ActiveStudy --> EngagedView: stay 25-90s
    ActiveStudy --> DeepStudy: stay > 90s
...
5. Построить `customer_data`.
6. Построить `supplier`.
7. Построить `ste` из каталога.
8. Добавить stub-СTЕ для 1 561 `ste_id`, встречающихся только в контрактах.
9. Построить `sale`.
10. Построить `ste_supplier_stat`.
11. Вычислить `ste.id_supplier`.
12. Построить индексы поиска и эмбеддинги.
...
4. Если у поставщика несколько регионов, брать наиболее частый и хранить все варианты в `region_variants`.
5. Все аномалии грузить в таблицу `etl_quality_log`.
...
2. В MVP подмешать только историю закупок и контрактные признаки.
3. Поведенческие сигналы оставить как расширение второй итерации после накопления реальных событий.
4. Подмешать affinity по категориям, атрибутам, производителям и поставщикам.
...
2. Можно честно объяснить пользователю, что группа построена:
   - по реальному бренду;
...
### 14.2 Контрактные позитивные сигналы
...
### 14.3 Контрактные негативные сигналы
...
### 14.4 Поведенческие сигналы для online-переобучения

| Сигнал | Правило | Эффект |
|---|---|---|
| `long_product_view` | Активный просмотр карточки > 45 сек | Небольшой boost группе/семейству |
| `deep_product_view` | Просмотр > 90 сек | Средний boost |
...
final_score =
  0.42 * lexical_score +
  0.15 * semantic_score +
  0.18 * purchase_affinity +
  0.10 * behavior_affinity +
  0.05 * supplier_affinity +
  0.10 * category_attribute_affinity
```
...
2. В live/demo-сценариях `behavior_affinity` включается сразу после поступления событий и пересчёта профиля.

### 14.7 Session-level дообучение
...
6. При следующем таком же запросе:
   ...
   - в ответе возвращается объяснение, почему ранжирование изменилось.
...
### 15.3 Online/demo-метрики поведенческой адаптации
...
5. `Behavior Explanation Precision` — доля кейсов, где explanation корректно указывает, какое событие изменило ранжирование.

### 15.4 Метрики полноты и устойчивости
...
### 15.5 Протокол оффлайн-проверки
...
### 15.6 Протокол demo-проверки поведения
...
6. Зафиксировать:
   ...
   - какие `reason_flags` и `explanations` вернул backend.

### 15.7 Что показывать на защите

1. Сравнение `baseline lexical` против `lexical + personalization`.
2. Сравнение `lexical + personalization` против `lexical + personalization + semantic rerank`.
3. Таблицу `HitRate@5`, `MRR@10`, `NDCG@10`, `Same Category HitRate@10`.
4. Кейс `Session A vs Session B` для одного пользователя и одного запроса.
5. Изменение позиций после длинного просмотра и после быстрого возврата.
6. Объяснение backend, почему одинаковый запрос вернул разную выдачу.
  ...
  "groups": [
    {
      ...
      "score": 0.8812,
      "explanations": [
        "Вы уже закупали товары этого производителя",
        "Похожие позиции долго изучались в прошлых сессиях"
      ],
      "reason_flags": [
        "PURCHASE_HISTORY",
        "LONG_VIEW",
...
3. Сессия 2:
   ...
   - в ответе присутствуют `reason_flags` и `explanations`.

---

## 18. Объяснимость результатов поиска

По ТЗ пользователь должен понимать, почему выдача изменилась.

Для этого в поисковом ответе должны возвращаться:

1. `reason_flags`
2. человекочитаемые `explanations`
3. источник группировки `group_type`
4. источник исправления запроса

Примеры объяснений:

1. "Результат поднят выше, потому что вы раньше закупали похожие СТЕ".
2. "Результат поднят выше, потому что вы долго изучали товары этой категории".
3. "Запрос исправлен: `резистар` -> `резистор`".
4. "Группа собрана по бренду из атрибута `Торговая марка`".
5. "Группа собрана по основному поставщику, так как производитель в данных не найден".
6. "В прошлой сессии вы 78 секунд изучали похожую группу товаров".
7. "Позиция понижена, потому что ранее был быстрый возврат к поиску".

---

## 19. Рекомендуемая реализация ML-части

### 19.1 Реалистичный вариант для хакатона
...
### 19.4 Что не стоит делать в первой версии
...
## 20. Frontend-архитектура Angular

### 20.1 Основные экраны

1. Регистрация.
2. Вход.
3. Поисковая страница.

### 20.2 Основные feature-store модули NgRx
...
### 20.3 Что должно быть на поисковой странице

1. Строка поиска.
2. Dropdown подсказок.
3. История запросов.
4. Подсказка с исправлением опечатки.
5. Группированная выдача по производителю/семейству.
6. Плашки объяснений, почему группа поднята выше.
...
## 22. Производительность и масштабирование

### 22.1 Что даст быстрый старт
...
### 22.2 Что можно масштабировать позже
...
5. Добавить quality-log и дедупликацию.
6. Поднять Redis для фоновых задач и кэша.
...
## 25. Открытые вопросы на подтверждение
